...
class Browser
{
    ...
    public function ProcessAjax() {
        ...
        if ($_POST['brtype'] == 'setheadfilter') {
            try {
                $v_oActDataField = $this->i_aFields[0];
    ...
    private function BuildSQLSelect($a_iLoadNum, $a_iSkip, &$a_aParams) {
        ...
        if (count_chars($v_sSQL) == $v_iPreLength) {
        ...
        if (count_chars($v_sSQL) == $v_iPreLength) {
            $v_sSQL = substr($v_sSQL, 0, -10); // odebereme order by
        }
        
        //Logging::WriteLog(LogType::Announcement, 'SQL: ' . $v_sSQL);
        return $v_sSQL;
    }
