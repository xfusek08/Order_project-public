...
class Deliverer extends DatabaseEntity
{
  ...
  protected function DefColumns()
  {
    ...
    $this->AddSQLColumn(DataType::Integer, 
      'ordlv_objnum', 'select count(1) from or_order where oror_raal = ordlv_raal and oror_isstorno != 1');
    $this->AddSQLColumn(DataType::Float, 'ordlv_obrat', 
      'select COALESCE (sum(oror_vydej), 0)  from or_order where oror_raal = ordlv_raal and oror_isstorno != 1');
    $this->AddSQLColumn(DataType::Float, 'ordlv_koeficient', 
    ...
    $this->AddSQLColumn(DataType::Float, 'ordlv_zisk', 
...
class Order extends DatabaseEntity
{
  ...
  protected function DefColumns()
  {
    ...
    $this->AddColumn(DataType::Integer, 'oror_cisloobjrok')->i_bUnformated = true;
