...
class DataField
{
  ...
  public function GetHeaderCell()
  {
    ...
    if ($this->i_DataType == DataType::String || count($this->i_aFields) > 0)
    {
      ...
      if ($this->i_iMaxlenght > 0)
        $v_sRes .= ' maxlength="' . $this->i_iMaxlenght . '" ';  
  ...
  public function GetValue()
  {
    ...
    switch ($this->i_DataType)
    {
      case DataType::String:
        return $this->i_sValue;
      case DataType::Integer:
        if ($this->i_bFormated)
          return number_format(intval($this->i_sValue), 0, '', ' ');
        return intval($this->i_sValue);
      case DataType::Float:
        return number_format(floatval($this->i_sValue), 2, ',', ' ');
      case DataType::Date:
        return date(DATE_FORMAT, strtotime($this->i_sValue));
      case DataType::DateTrnc:
        return date('d.m.', strtotime($this->i_sValue));
      case DataType::Timestamp:
        return date(DATE_TIME_FORMAT, strtotime($this->i_sValue));
      case DataType::Bool:
        return $this->i_sValue;
    }
  }
  public function GetSelectCondition(&$a_aParams)
  {
    ...
    else if (($this->i_DataType == DataType::Integer || $this->i_DataType == DataType::Float) && $this->i_iFilterNumber !== null)
    {
      ...
      {
        case Operator::Equal:       $opr = '=';   break;
        case Operator::IsBigger:    $opr = '>';   break;
        case Operator::IsBiggerEg:  $opr = '>=';  break;
        case Operator::IsSmaller:   $opr = '<';   break;
        case Operator::IsSmallerEq: $opr = '<=';  break;
      }
      if ($opr != '')
      {
