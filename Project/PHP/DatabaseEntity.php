    ...
    public function GetAsXML($formatted = true) {
        ...
        for ($i = 0; $i < $this->i_iColCount; $i++) {
            $res .=
                ...
                $this->i_aColumns[$i]->GetValueAsString($formatted) .
                '</' . strtolower($this->i_aColumns[$i]->i_sName) . '>';
        }
        $res .= '</' . strtolower($this->i_sTableName) . '>';
        return $res;
...
class DBEntColumn
{
    ...
    public function GetValueAsString($a_bFormatted = true) {
        if ($this->GetValue() === null) {
            return '';
        }
        
        switch ($this->i_tDataType) {
            case DataType::String:    return $this->GetValue();
            case DataType::Date:      return date(DATE_FORMAT, $this->GetValue());
