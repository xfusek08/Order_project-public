...
class SettingPage extends Page
{
    ...
    <div class="settingpage">
      ...
      <form method="post">
        ...
        <table>
          <tr>
            ...
            <td>
              <input type="text" name="orset_cisloobj"
                value="<?php echo $this->i_oSetting->GetColumnByName('orset_cisloobj')->GetValue(); ?>"/>
            </td>
            <td>(Lze měnit. Roste s každou objednávkou.)<td>
          </tr>
          <tr>
            <td>Rok objednávky:</td>
            <td>
              <input type="text" name="orset_cisloobjrok"
                value="<?php echo $this->i_oSetting->GetColumnByName('orset_cisloobjrok')->GetValue(); ?>"/>
            </td>
            <td>(Nutno ručně aktualizovat)<td>
          </tr>
        </table>
      ...
      <?php 
        if (isset($_SESSION['importsummary']))
          echo '<textarea readolny style="width: 100ch; height: 50ch">' . $_SESSION['importsummary'] . "</textarea>";                  
      ?>
    </div>    
    <?php
  }
  public function ProccessPost()
  {
    if (isset ($_POST['yearnum']))
    {
      if (isset ($_POST['c_submit']))
      {
        ...
        if ($this->i_oSetting->SaveToDB(false))
          ...
          $this->AddAlert('red', 'Běhěm ukládání nastala chyba.');
     }
    }
    if (isset ($_POST['dataimport']))
    {
      ...
      else
      {
        ...
        {
          $this->AddAlert('red', 'Import neproběhl vpořádku.');        
        }
        else
        {
          $this->AddAlert('green', 'Import proběhl vpořádku.');                
        }
  ...
  {
    Logging::WriteLog(LogType::Anouncement, 'OrderXMLImport() ...');
    ...
    finally
    {
      ...
      Logging::WriteLog(LogType::Anouncement, 'OrderXMLImport() Ended.');
      ...
      {
        $a_sImportsummary .= "Import succeded:" . PHP_EOL;
  ...
  public function ImportOneOrderFromElement($a_oXmlOrder, &$a_sResDesc)
  {
    ...
    Logging::WriteLog(LogType::Anouncement, 'Saving order: ' . $v_oOrderForm->i_oOrder->GetColumnByName('oror_cisloobj')->GetValueAsString());
    ...
    if ($validdata)
    {
      $STDBR = $v_oOrderForm->SaveFullOrderToDB();
