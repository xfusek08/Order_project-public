...
$(document).ready(function () {
    if ($('.browser').length > 0) {
        ...
        $('body').keydown(function (e) {
            if (!$('form *:focus').length
                && !$('headcol *:focus').length
                ...
                if (e.keyCode == 38) { // up arrow
                    e.preventDefault();
                    index--;
                    if (index >= 0 && typeof OnBrowseRowChangeCallBack === 'function') {
                        OnBrowseRowChangeCallBack(index);
                    }
                } else if (e.keyCode == 40) { // down arrow
                    e.preventDefault();
                    index++;
                    if ((index) < $('.browser table tr').length && typeof OnBrowseRowChangeCallBack === 'function') {
                        OnBrowseRowChangeCallBack(index);
                    }
    ...
    if (newTabHead.length === 0 || newTabBody.length === 0) {
        ...
        newTabBody.scroll(function () {
            ...
            if (newTabBody.scrollTop() >= (newTabBody.prop('scrollHeight') - newTabBody.height())) {
                RequestNextData(50, newTabBody.find('tr').size());
            }
        });
    } else {
...
function SelectRow(pk) {
    ...
    if ($('.browser table tr[pk=' + pk + ']').length > 0) {
        ...
        RequestNextData(0, $('.browser tbody tr').size(), function () {
            $('.browser table tr[pk=' + pk + ']').addClass('selected');
            ScrollOnSelectedRow();
        });
    }
...
function ScrollOnSelectedRow() {
    ...
    var scrollContainer = $('.browser > div:eq(1)');
    var selectedRow = $('.browser > div:eq(1) tr.selected');
    var scrollTop = scrollContainer.scrollTop();
    var bottomOffset = 0;
    if (scrollContainer.prop('scrollWidth') > scrollContainer.width()) {
        bottomOffset = 20;
    }
    
    if (selectedRow.length != 0) {
        if (selectedRow.position().top + selectedRow.outerHeight() - headHeight + bottomOffset > scrollContainer.height()) {
            scrollContainer.scrollTop(selectedRow.position().top - scrollContainer.height() - headHeight + bottomOffset + selectedRow.outerHeight() + scrollTop);
        } else if (selectedRow.position().top - headHeight + 1 < 0) {
            scrollContainer.scrollTop(scrollTop + selectedRow.position().top - headHeight);
        }
    }
}
...
function SendDateFilterReq(v_oDateFilter) {
    ...
    SendFilterRequest(
        HeadcolIdent,
        'date',
        'datefrom=' + v_sDateFrom + '&dateto=' + v_sDateto
    );
}
        ...
        SendAjaxRequest(
            ...
            function (response) {
                if ($(response).attr('state') == 'ok') {
                    ...
                    g_AnnouncementManager.AddAnnouncement('red', 'Chyba odeslání filtru');
                }
            }
        );
...
function CheckScreenLoaded(a_oForm) {
    ...
    if ($('.browser > div:eq(1)').prop('scrollHeight') <= $('.browser > div:eq(1)').height()) {
        RequestNextData(50, $('.browser tbody tr').size());
    }
}

function RequestNextData(load, skip, callback) {
        ...
        SendAjaxRequest(
            ...
            function (response) {
                if ($(response).attr('state') == 'ok') {
                    ...
                    g_AnnouncementManager.AddAnnouncement('red', 'Chyba načítání dalších dat');
                }
            }
        );
...
function ClearFiltersRequest() {
    SendAjaxRequest(
        'type=browser' + '&brtype=resetfilters',
        true,
        function (response) {
            if ($(response).attr('state') == 'ok') {
                var v_iScrollTop = 0;
...
function AddSummaryToBrowser(v_oBrowser) {
    if (v_oBrowser.attr('showsummary') !== '1') {
        return;
    }
    
    SendAjaxRequest(
        'type=browser' + '&brtype=getSummary',
        true,
        function (response) {
            var append = false;
